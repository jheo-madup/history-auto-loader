...
10. 자동입찰시트의 `키워드`/`목표순위` 스냅샷을 비교해 당일 목표순위 변경을 Raw/요약에 추가합니다.
...
자동입찰시트는 Google Sheets API로 읽고, `키워드`와 `목표순위`/`목표 순위` 컬럼을 기준으로 직전 실행 스냅샷과 비교합니다.
...
스냅샷은 히스토리 스프레드시트의 `_자동입찰_snapshot_state` 탭에 저장합니다. 최초 실행은 기준 스냅샷만 저장하고, 이후 실행부터 목표순위 변경을 감지합니다.
운영 시트처럼 헤더가 `캠페인명`, `광고그룹명`, `목표 순위`로 되어 있어도 자동 인식합니다.
...
Raw에는 `변경유형=자동입찰 목표순위 변경`, `변경필드=목표순위`, `변경레벨=키워드`로 적재합니다.
...
자동입찰 스냅샷 탭 `_자동입찰_snapshot_state`는 코드가 자동 생성하며, 가능하면 hidden 처리합니다.
